...
PORT = 5000  # Match your Cloudflare Tunnel port
...
MUSIC_DIR = "/volume1/homes/benjamin/music"  # Update to your Synology music folder or container path

USER_SESSIONS = {}


def download_audio_with_ytdlp(target_url_or_query, output_dir=MUSIC_DIR):
    """Downloads audio matching a YouTube/Invidious URL or search query and converts to mp3 via ffmpeg."""
    os.makedirs(output_dir, exist_ok=True)
    out_tmpl = os.path.join(output_dir, "%(title)s.%(ext)s")

    # If it's not a direct URL, format as a YouTube search query
    if not target_url_or_query.startswith("http"):
                            ...
                            if match:
                                ...
                                if filename:
                                    ...
                                    with open(save_path, "wb") as f:
                                        f.write(file_data)
                                    self.send_html(f"<h3>Successfully uploaded '{filename}' to music directory!</h3><p><a href='/downloader/'>Back to Hub</a></p>")
                                    return
    ...
    def do_GET(self):
        ...
        if path in ['', '/downloader']:
            ...
            if search_query:
                try:
                    ...
                    if req.status_code == 200:
                        results = req.json()[:5]
                        search_results_html = "<h4>Search Results (via yt.jobbisoft.com.mx):</h4><ul style='list-style:none; padding:0;'>"
                        for item in results:
            ...
            <head>
                ...
                <title>Music Downloader Hub</title>
                <style>
                    :root {{
                        --bg-color: #121212;
                        --card-bg: #1e1e1e;
            ...
            <body>
                <h2>Music Server Downloader & Upload Hub</h2>
                ...
                <div class="card">
                    ...
                    <form action="/downloader/" method="GET">
                        ...
                        <input type="text" name="q" value="{search_query}" placeholder="Search via yt.jobbisoft.com.mx..." required>
                ...
                <!-- Section 3: Spotify Downloader Form -->
                <div class="card">
                    <h3>Spotify Track Downloader</h3>
                    <form action="/downloader/auth" method="GET">
                        <label>Spotify Client ID:</label>
                        <input type="text" name="client_id" required placeholder="Paste Client ID">
                        
                        <label>Spotify Client Secret:</label>
                        <input type="password" name="client_secret" required placeholder="Paste Client Secret">
        ...
        # 2. Direct Download Trigger
            ...
            video_url = f"{INVIDIOUS_INSTANCE}/watch?v={video_id}"
            success = download_audio_with_ytdlp(video_url)

            if success:
                self.send_html(f"<h3>Successfully downloaded: {title}</h3><p><a href='/downloader/'>Back to Hub</a></p>")
            else:
                self.send_html(f"<h3>Failed to download track from Invidious.</h3>", status_code=500)
        ...
        # 4. Spotify Callback
            ...
            track_req = requests.get(f"https://api.spotify.com/v1/tracks/{track_id}", headers=headers)
            ...
            if track_req.status_code == 200:
                ...
                if success:
                    self.send_html(f"<h3>Successfully downloaded: {search_query}</h3><p><a href='/downloader/'>Back to Hub</a></p>")
                else:
